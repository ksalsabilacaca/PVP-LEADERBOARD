...
public class JoinNpcManager {
    ...
    public void spawnFromConfig() {
        if (!plugin.getConfig().getBoolean("npc.enabled", false)) return;
    ...
    private void spawnHologram(Location base) {
        hologramLine1 = spawnLine(base.clone().add(0, 2.85, 0), ChatColor.GREEN + "" + ChatColor.BOLD + "CLICK TO PLAY");
    ...
    private void startFacingTask(Location base) {
        if (faceTask != null) faceTask.cancel();
    ...
    public void remove() {
        ...
        npc = null;
        if (hologramLine1 != null && !hologramLine1.isDead()) hologramLine1.remove();
        if (hologramLine2 != null && !hologramLine2.isDead()) hologramLine2.remove();
        if (hologramLine3 != null && !hologramLine3.isDead()) hologramLine3.remove();
        hologramLine1 = null;
        hologramLine2 = null;
        hologramLine3 = null;
        ...
        for (World world : plugin.getServer().getWorlds()) {
            for (Entity entity : world.getEntities()) {
                if (entity.getPersistentDataContainer().has(plugin.getJoinNpcKey(), PersistentDataType.STRING)
                        || entity.getPersistentDataContainer().has(plugin.getJoinNpcHologramKey(), PersistentDataType.STRING)) {
                    entity.remove();
                }
            }
        }
    ...
    public boolean isJoinNpc(Entity entity) {
        return entity != null && entity.getPersistentDataContainer().has(plugin.getJoinNpcKey(), PersistentDataType.STRING);
    }
}
